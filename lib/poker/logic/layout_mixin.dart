...
mixin LayoutMixin {
  static const double _percentK = .3; // 滑动超出宽高范围，计算百分比使用
  ...
  // 滑动百分比为[0,1]
  double swipePercent(bool x, Offset dif, Rect rect) {
    if (rect.isEmpty) {
      return 0;
    } else {
      if (x) {
        return math.min(1, dif.dx.abs() / (rect.shortestSide * _percentK));
      } else {
        return math.min(1, dif.dy.abs() / (rect.shortestSide * _percentK));
      }
    }
  }

  double rotate(Offset dif, Rect rect, bool up) =>
      swipePercent(true, dif, rect) * _maxRotate * (up ? 1 : -1) * (dif.dx >= 0 ? 1 : -1);
